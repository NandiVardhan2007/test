...
--what is tthe max salary that is being earned by an employee in my org
select max(salary) from employee;

--who is earning that max salary
select name from employee where salary=85000;
select name,salary from employee order by salary desc;

--subquries
--a query written inside another query
--1.scalary subquery(a subquery that returns value in a single row and single column)
...
--display the id,name,ageof employee whomis the youngest in the organisation
select min(age) from employee;
select id,name,age from employee where age =(select min(age) from employee);
--tell me atleast 3 names who are earning more thean the average salary of all the employees in the organisation
select avg(salary) from employee;
SELECT id,name
FROM employee
WHERE salary > (SELECT AVG(salary) FROM employee)
FETCH FIRST 3 ROWS ONLY;
--find out the how many people from hr department are earning more than the avg salry   of all tthe employee from operations departmrnt

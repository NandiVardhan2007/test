...
-- DROP TABLE star_rating CASCADE CONSTRAINTS;
...
SELECT
    SUM(CASE WHEN sr.star = 3 THEN 1 ELSE 0 END) AS star_3,
    SUM(CASE WHEN sr.star = 4 THEN 1 ELSE 0 END) AS star_4,
    SUM(CASE WHEN sr.star = 5 THEN 1 ELSE 0 END) AS star_5,
    SUM(CASE WHEN sr.star = 6 THEN 1 ELSE 0 END) AS star_6,
    SUM(CASE WHEN sr.star = 7 THEN 1 ELSE 0 END) AS star_7
FROM users u
JOIN star_rating sr
ON u.contest_rating BETWEEN sr.min_rating AND sr.max_rating;
  SELECT name
FROM coders
WHERE stars = 7
  AND doj = (
...
SELECT COUNT(*) AS total_3star_2022
FROM users u
JOIN star_rating sr
ON u.contest_rating BETWEEN sr.min_rating AND sr.max_rating
WHERE sr.star = 3
AND EXTRACT(YEAR FROM u.doj) = 2022;


SELECT name, dob
FROM (
    SELECT u.name, u.dob,
           ROW_NUMBER() OVER (ORDER BY u.dob DESC) AS youngest_rank,
           ROW_NUMBER() OVER (ORDER BY u.dob ASC) AS oldest_rank
    FROM users u
    JOIN star_rating sr
    ON u.contest_rating BETWEEN sr.min_rating AND sr.max_rating
    WHERE sr.star = 2
...
WHERE youngest_rank = 1
   ...
   SELECT SUM(u.contest_rating) AS total_6star_rating
FROM users u
JOIN star_rating sr
ON u.contest_rating BETWEEN sr.min_rating AND sr.max_rating
WHERE sr.star = 6;
...
SELECT AVG(u.contest_rating) AS avg_5star_rating
FROM users u
JOIN star_rating sr
ON u.contest_rating BETWEEN sr.min_rating AND sr.max_rating
WHERE sr.star = 5;

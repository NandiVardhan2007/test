...
SELECT MAX(date_of_admission) AS most_recent_admission
FROM student
WHERE branch = 'CSE'
AND college = 'ACOE';
...
select count(fee) from student where  college='AEC' or college='ACOE';

SELECT college, SUM(fee) AS total_fee
FROM student
WHERE college IN ('AEC', 'ACOE')
GROUP BY college;
...
FROM student
WHERE branch NOT IN ('CSE', 'IT', 'EEE', 'DS');
...
SELECT COUNT(*) AS students_joined
FROM student
WHERE admission_date BETWEEN 
      DATE 01-07-2023 AND DATE '31-12-2023';


SELECT COUNT(*) AS students_joined
FROM student
WHERE admission_date BETWEEN '2023-07-01' AND '2023-12-31';
...
SELECT date_of_admission, COUNT(*) AS total_students
FROM student
